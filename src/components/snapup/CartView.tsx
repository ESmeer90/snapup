...
import { SERVICE_FEE_RATE } from '@/lib/payfast';
...
const CartView: React.FC<CartViewProps> = ({ onViewDetail, onViewChange, onCheckoutCart }) => {
  ...
  // Show all items, even those with missing listing data (graceful degradation)
  // Items with listing data AND active status
  const activeItems = cartItems.filter(i => i.listing && i.listing.status === 'active');
  // Items where listing data is missing (join failed, data loading issue, etc.)
  const orphanedItems = cartItems.filter(i => !i.listing);
  // Items where listing is no longer active (sold, deleted, etc.)
  const unavailableItems = cartItems.filter(i => i.listing && i.listing.status !== 'active');
  // All displayable items (active + orphaned shown with fallback UI)
  const displayItems = [...activeItems, ...orphanedItems];

  // Remove unavailable items automatically (only those we KNOW are unavailable, not orphaned ones)
  useEffect(() => {
  ...
  // Calculate totals (use activeItems for pricing, since orphaned items have no price data)
  const subtotal = activeItems.reduce((sum, item) => {
    const price = typeof item.listing?.price === 'number' ? item.listing.price : 
                  typeof item.listing?.price === 'string' ? parseFloat(item.listing.price) || 0 : 0;
    return sum + price * item.quantity;
  }, 0);
  const totalServiceFee = Math.round(subtotal * SERVICE_FEE_RATE);
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      {!cartLoading && displayItems.length > 0 && (
