...
import { formatZAR, timeAgo, sendMessage } from '@/lib/api';
...
import {
  ...
  Wallet, ShoppingBag, ShoppingCart, Info, ExternalLink, CheckCircle2,
  Bell, BellRing, Bookmark, AlertTriangle
} from 'lucide-react';
...
const ListingDetail: React.FC<ListingDetailProps> = ({ listing, onClose, isFavorited, onToggleFavorite, onStartChat, onViewSellerProfile }) => {
  const { user } = useAuth();
  const cart = useCart();
      ...
      <div className="grid md:grid-cols-2">
        ...
        <div className="p-6 flex flex-col">
          <div className="flex-1">
            ...
            {/* Seller Earnings (for own listings) */}
            {isOwnListing && (
          ...
          <div className="mt-6 space-y-3">
            ...
            {user && !isOwnListing && listing.status === 'active' && onStartChat && (
              <button onClick={handleOpenChat} className="w-full py-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-200 flex items-center justify-center gap-2.5 text-base group">
            ...
            {/* Add to Cart */}
            {user && !isOwnListing && listing.status === 'active' && cart && (
              <button
