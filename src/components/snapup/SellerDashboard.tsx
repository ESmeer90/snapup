...
const SellerDashboard: React.FC<SellerDashboardProps> = ({ onViewChange }) => {
  ...
  const [refundModal, setRefundModal] = useState<{
    ...
    listingTitle: string;
    listingImage?: string;
  }>({ open: false, orderId: '', orderAmount: 0, orderTotal: 0, listingTitle: '' });
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
                          ...
                          <div className="flex items-start gap-3 sm:gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-3 flex-wrap">
                                ...
                                {/* Refund Button - for paid/shipped/delivered orders */}
                                {['paid', 'shipped', 'delivered'].includes(order.status) && (
                                  <button
                                    ...
                                    className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-all"
                                  >
      ...
      <RefundModal
        ...
        onClose={() => setRefundModal({ open: false, orderId: '', orderAmount: 0, orderTotal: 0, listingTitle: '' })}
        orderId={refundModal.orderId}
        orderAmount={refundModal.orderAmount}
        orderTotal={refundModal.orderTotal}
        listingTitle={refundModal.listingTitle}
        listingImage={refundModal.listingImage}

        onRefundProcessed={loadData}
      />
    </section>
