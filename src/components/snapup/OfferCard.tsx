...
import { formatZAR, updateOfferStatus, createOffer, sendMessage, getOrderForOffer } from '@/lib/api';
import type { Offer, OfferStatus, Order } from '@/types';
import { OFFER_STATUS_CONFIG } from '@/types';
import {
  Tag, CheckCircle2, XCircle, ArrowLeftRight, Clock, Undo2,
  Loader2, ChevronDown, ChevronUp, CreditCard, Wallet
} from 'lucide-react';
...
interface OfferCardProps {
  ...
  onPayNow?: (offer: Offer, order: Order) => void;
}
...
const OfferCard: React.FC<OfferCardProps> = ({
  ...
  const agreedAmount = offer.counter_amount || offer.amount;
  ...
  // ===== PAY NOW HANDLER (buyer only, accepted offers) =====
  const handlePayNow = async () => {
    if (!onPayNow) return;
    setPayNowLoading(true);
    try {
      const pendingOrder = await getOrderForOffer(offer.id);
      if (!pendingOrder) {
        toast({
          title: 'Order not found',
          description: 'The pending order for this offer could not be found. The seller may need to re-accept.',
          variant: 'destructive',
        });
        return;
      }
      onPayNow(offer, pendingOrder);
    } catch (err: any) {
      toast({ title: 'Error', description: err.message || 'Failed to load order for payment', variant: 'destructive' });
    } finally {
        ...
        <div className={`rounded-2xl overflow-hidden border-2 shadow-sm ${
              ...
              <div className="mt-3 p-2.5 bg-emerald-100/60 rounded-xl border border-emerald-200">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-xs font-bold text-emerald-700">
                    Deal agreed at {formatZAR(agreedAmount)}
                  </p>
                </div>
              </div>
            )}
          ...
          {!loading && (
            <div className="px-4 pb-3">
              ...
              {/* ===== PAY NOW BUTTON (buyer only, accepted offers) ===== */}
              {isBuyer && isAccepted && (
                <button
                  onClick={handlePayNow}
                  disabled={payNowLoading}
                  className="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-blue-200 flex items-center justify-center gap-2 disabled:opacity-60"
                >
                  {payNowLoading ? (
                  ...
                  {payNowLoading ? 'Loading...' : `Pay Now — ${formatZAR(agreedAmount)}`}
                </button>
              )}

              {/* Seller: accepted offer info */}
              {isSeller && isAccepted && (
