...
import {
  ...
  getListing, fetchUserRatingsBatch, getOffersForConversation, createOffer, acceptOfferAndCreateOrder
} from '@/lib/api';
import type { Conversation, Message, Listing, Offer, Order } from '@/types';
...
const MessagesView: React.FC<MessagesViewProps> = ({ initialChat, onClearInitialChat, onViewListing, onBuyNow }) => {
  ...
  const handleAcceptAndBuy = async (acceptedOffer: Offer) => {
    if (!chatListing || !user || !activeConv) return;

    // Determine the agreed price
    const agreedPrice = acceptedOffer.counter_amount || acceptedOffer.amount;
    const { buyerId, sellerId } = getConversationRoles();

    // If seller is accepting → create order + notify buyer
    if (user.id === sellerId) {
      try {
        // acceptOfferAndCreateOrder handles: update offer status, cancel stale orders, create pending order, notify buyer
        const result = await acceptOfferAndCreateOrder(
          acceptedOffer.id,
          chatListing,
          buyerId,
          sellerId,
        ...
        // Send a chat message to buyer with payment link
        try {
          const payMsg = `✅ Offer accepted! Deal agreed at ${formatZAR(agreedPrice)}. Tap the "Pay Now" button on the offer card above to complete your purchase.`;
          const sent = await sendMessage(
            activeConv.listing_id,
            user.id,
            activeConv.other_user_id,
        ...
        toast({
          ...
          description: `Deal agreed at ${formatZAR(agreedPrice)}. The buyer has been notified to complete payment.`,
        });
      } catch (err: any) {
        console.error('[handleAcceptAndBuy] Error:', err);
        toast({
          title: 'Error',
          description: err.message || 'Failed to process accepted offer',
          variant: 'destructive',
        });
      }
    } else {
      // Buyer accepted a counter-offer → send chat message + trigger checkout
      try {
        const acceptMsg = `✅ Counter offer accepted! Deal agreed at ${formatZAR(agreedPrice)}.`;
        const sent = await sendMessage(
          activeConv.listing_id,
          user.id,
          activeConv.other_user_id,
      ...
      // Create order for the accepted counter
      try {
        await acceptOfferAndCreateOrder(
          acceptedOffer.id,
          chatListing,
          buyerId,
          sellerId,
        ...
        console.warn('[handleAcceptAndBuy] Order creation for counter-accept failed:', err);
      }

      // Trigger checkout at agreed price
      if (onBuyNow) {
  ...
  // ===== PAY NOW HANDLER (from OfferCard) =====
  const handlePayNowFromOffer = (offer: Offer, order: Order) => {
    if (!chatListing || !onBuyNow) return;
    const agreedPrice = offer.counter_amount || offer.amount;
    // Create a listing copy with the offer price for checkout
    const listingAtOfferPrice: Listing = {
      ...chatListing,
      price: agreedPrice,
    };
                ...
                return (
                  <React.Fragment key={`offer-${offer.id}`}>
                    ...
                    <OfferCard
                      ...
                      onPayNow={handlePayNowFromOffer}
                    />
